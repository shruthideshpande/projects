   ...
   NODE_MSG_HDR_X *px_msg_header)
{
   ...
   if (ui_index < px_dimutex_ctxt->x_init_params.ui_no_nodes)
   {
      ...
      for (ui_index = 0; ui_index < px_dimutex_ctxt->x_init_params.ui_no_nodes;
               ui_index++)
      {
         if (px_dimutex_ctxt->x_nodes.xa_nodes [ui_index].e_state !=
               eDIMUTEX_NODE_STATE_ALGO_READY)
         {
...
{
   DIMUTEX_RET_E e_error = eDIMUTEX_RET_FAILURE;
   NODE_MSG_ALGO_COMPLETE_X *px_complete = NULL;
   uint32_t ui_index = 0;
   ...
   px_complete = (NODE_MSG_ALGO_COMPLETE_X *) px_msg_header;
   ...
   if (ui_index < px_dimutex_ctxt->x_init_params.ui_no_nodes)
   {
      ...
      e_error = dimutex_node_aggregate_stats (px_dimutex_ctxt, ui_index,
         &(px_complete->x_stats));
      if (eDIMUTEX_RET_SUCCESS != e_error)
      {
         DMUT_LOG_LOW("dimutex_node_aggregate_stats failed: %d", e_error);
      }
   }
   else
   {
   ...
   NODE_MSG_HDR_X *px_msg_header)
{
   ...
   if ((NULL == px_dimutex_ctxt) || (NULL == px_msg_header))
   {
      DMUT_LOG_LOW("Invalid Args");
      e_error = eDIMUTEX_RET_INVALID_ARGS;
      goto CLEAN_RETURN;
