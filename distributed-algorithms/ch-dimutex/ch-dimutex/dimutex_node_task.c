   ...
   DIMUTEX_MSG_HDR_X *px_msg_hdr)
{
   ...
   e_error = dimutex_node_aggregate_stats (px_dimutex_ctxt, 0,
      &(px_algo_complete->x_stats));
   if (eDIMUTEX_RET_SUCCESS != e_error)
   {
   ...
   DIMUTEX_CTXT_X *px_dimutex_ctxt)
{
   ...
   DMUT_LOG_LOW("Me: %s (0x%x):%d", px_cur_node->uca_node_dns_name_str,
      px_cur_node->ui_node_ip_addr_ho, px_cur_node->us_node_listen_port_ho);
   e_error = eDIMUTEX_RET_SUCCESS;
CLEAN_RETURN:
   return e_error;
}
